...
def build(target, configuration, trial)
	command = "xcodebuild -project #{PROJECT} -sdk #{SDK} -configuration #{configuration} -arch i386 -arch x86_64 -target #{target}"
	system command
...
build_lib("ObjectiveChipmunk", ["Objective-Chipmunk/*.h", "Chipmunk/include/chipmunk"])
